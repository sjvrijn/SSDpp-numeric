...
for datasetname in datasetnames:
    ...
    dfaux = pd.read_csv(file_data,sep =delimiter)
    #dfaux, test = train_test_split(df, test_size=testpercentage,random_state = np.random.seed(0))
    dataset = dfaux.to_dict(orient ="records")
    new_dataset=deepcopy(dataset)   
    # change configuration file of DSSD
    conf_file=read_csvfile('./otheralgorithms/DSSD/bin/tmpModelphase2.conf')
    ...
    dfaux.rename(columns={ dfaux.columns[-1]: "class" }, inplace = True)
    ...
    subgroup_sets_usage= []
    statistic_rules = []
    yvaluestrain = dfaux["class"].to_numpy()
    for r in range(len(subgroup_sets_support)):
        ...
        if len(auxset)>1:
            values = yvaluestrain[list(auxset)]
            rulestat = {"mean":np.mean(values), "variance": np.var(values)}
            #if rulestat["mean"] ==np.nan or rulestat["variance"] ==np.nan:
            #    toremove.append(r)    
        else:
            values = 0
    ...
    print("measures wkl: " + str(measures["wkl_sum"]))
